 ...
 * @author mlr
 */
public class N_XC60_DELETE extends AbstractCompiler2 {
	private java.util.Hashtable m_methodReturnHas = new java.util.Hashtable();
	private Hashtable m_keyHas = null;


	public N_XC60_DELETE() {
		super();
	}
	...
	public Object runComClass(PfParameterVO vo) throws BusinessException {
		try {
			...
			Object retObj = null;
			// 方法说明:行业公共删除
			QualityGradeHVO hvo=(QualityGradeHVO) vo.m_preValueVo.getParentVO();

			boolean isReference = ZMReferenceCheck.getReferenceCheck().isReferenced(
					"xcgl_qualitygrade_h", vo.m_preValueVo.getParentVO().getPrimaryKey());
			if (isReference == true) {
				throw new BusinessException("数据已经被引用,编码为：" + "['"
						+ hvo.getVqualitygradecode() + "'] 名称为：['"
						+ hvo.getVqualitygradename() + "']");
			}
