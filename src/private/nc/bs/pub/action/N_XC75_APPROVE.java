 ...
 * @author jay
 */
public class N_XC75_APPROVE extends AbstractCompiler2 {
	...
	public Object runComClass(PfParameterVO vo) throws BusinessException {
		try {
			...
			List<GenPrcOutBVO> list= (List<GenPrcOutBVO>) getDao().retrieveByClause(GenPrcOutBVO.class, "pk_general_b='"+vlastbillrowid+"' and isnull(dr,0)=0");
			if(list!=null&&list.size()!=0){
				GenPrcOutBVO  bvo=list.get(0);
				UFDouble nwetweight=PuPubVO.getUFDouble_ValueAsValue(bvo.getAttributeValue("nwetweight"));
				UFDouble ndryweight=nwetweight.sub(nwetweight.multiply(ngrade.div(100)));
				String sql=" update xcgl_general_b set nwatercontent ='"+ngrade+"'"+", ndryweight='"+ndryweight+"'"+ " where pk_general_b='"+vlastbillrowid+"'";
