...
//		getbtnLock().setEnabled(isEdit||!isSign);
	...
	public void loadHeadData() {
		try {
			...
			//遍历map，map的值是数组，数组的第一个元素是表头vo组成的list只有一个vo
			//数组的二个元素是相对应的表体vo组成的list，个数不确定	
			...
			//setBodys((CircularlyAccessibleValueObject[])list[1].toArray(new SuperVO[0]));
			}
			else{
				this.myClientUI.showWarningMessage("该单据还没有进行分摊!");
			}
			}
//			if (billvo != null) {
...
//	    	if(vo.getSonVOS()!=null&&vo.getSonVOS().size()>0)
	    	...
	    	getbillListPanel().setBodyValueVO((CircularlyAccessibleValueObject[]) bvos);
